...
// --- SONG PLUS DMI (byd_song.dbc) ---
//
// The SONG family uses a different ADAS suite: openpilot impersonates the camera and
// sends the LKAS torque command on ACC_MPC_STATE (bus 0), while faking ACC_EPS_STATE
// back to the camera on bus 2 so it stays healthy (no DTC, AEB preserved). The buses
// are split statically: bus 0 keeps ACC_EPS_STATE, bus 2 keeps ACC_MPC_STATE/ACC_CMD.
//
// LKAS_Output is an 11-bit signed torque target (STEER_MAX = 300 raw), so this mode
// uses the torque checks with a fixed limit, not the ATTO3 angle checks.

#define BYD_SONG_ACC_MPC_STATE   0x316U  // 790 - LKAS torque command, bus 0
#define BYD_SONG_ACC_EPS_STATE   0x318U  // 792 - EPS state, bus 0; faked back on bus 2
#define BYD_SONG_ACC_HUD_ADAS    0x32DU  // 813 - ACC main/standby state, camera on bus 2
#define BYD_SONG_ACC_CMD         0x32EU  // 814 - ACC command, bus 0
#define BYD_SONG_CARSPEED        0x121U  // 289 - dash speed, bus 0
#define BYD_SONG_PEDAL           0x342U  // 834 - GAS_PEDAL/BRAKE_PEDAL, bus 0

#define BYD_SONG_FLAG 0x4U  // BydSafetyFlags.SONG_PLUS_DMI

static bool byd_song_mode = false;            // selected in byd_init via safetyParam
static bool byd_eps_cruiseactivated = false;  // EPS CruiseActivated from ACC_EPS_STATE
...
static safety_config byd_init(uint16_t param) {
  // SONG PLUS DMI shares this safety model with the ATTO3; safetyParam selects the
  // platform (BydSafetyFlags.SONG_PLUS_DMI = 0x4) so one mode serves both.
  byd_song_mode = (param & BYD_SONG_FLAG) != 0U;

  if (byd_song_mode) {
    // The LKAS torque command is safety-checked (check_relay keeps stuck-relay
    // detection); ACC_CMD and the faked ACC_EPS_STATE are plain repeats. Forwarding is
    // split statically in byd_fwd_hook, so no disable_static_blocking here.
    static const CanMsg BYD_SONG_TX_MSGS[] = {
      {(int)BYD_SONG_ACC_CMD,       0, 8, .check_relay = false},
      {(int)BYD_SONG_ACC_MPC_STATE, 0, 8, .check_relay = true},
      {(int)BYD_SONG_ACC_EPS_STATE, 2, 8, .check_relay = false},
    };

    // None of these carry a checksum or counter panda can verify.
    static RxCheck byd_song_rx_checks[] = {
      {.msg = {{(int)BYD_SONG_PEDAL,         0, 8, 50U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, {0}, {0}}},
      {.msg = {{(int)BYD_SONG_CARSPEED,      0, 8, 50U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, {0}, {0}}},
      {.msg = {{(int)BYD_SONG_ACC_EPS_STATE, 0, 8, 50U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, {0}, {0}}},
      {.msg = {{(int)BYD_SONG_ACC_HUD_ADAS,  2, 8, 50U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, {0}, {0}}},
    };

    byd_eps_cruiseactivated = false;

    safety_config ret;
    SET_TX_MSGS(BYD_SONG_TX_MSGS, ret);
    SET_RX_CHECKS(byd_song_rx_checks, ret);
    return ret;
  }
...
static void byd_rx_hook(const CANPacket_t *msg) {
  ...
  if (byd_song_mode) {
    if (msg->bus == 0U) {
      if (addr == BYD_SONG_PEDAL) {
        gas_pressed = msg->data[0] != 0U;                    // GAS_PEDAL : 0|8@1+
        brake_pressed = msg->data[1] != 0U;                  // BRAKE_PEDAL : 8|8@1+
      } else if (addr == BYD_SONG_CARSPEED) {
        // CarDisplaySpeed : 0|12@1+ (km/h; carstate applies the dash-speed scaling)
        uint8_t speed_lsb = msg->data[0];
        uint8_t speed_msb = (uint8_t)(msg->data[1] & 0xFU);
        vehicle_moving = (speed_lsb > 0U) || (speed_msb > 0U);
      } else if (addr == BYD_SONG_ACC_EPS_STATE) {
        byd_eps_cruiseactivated = GET_BIT(msg, 1U);          // CruiseActivated : 1|1@0+
        int torque_motor = (int)((msg->data[2] & 0xFU) << 8) | (int)msg->data[1];  // MainTorque : 8|12@1+
        torque_motor = to_signed(torque_motor, 12);
        update_sample(&torque_meas, torque_motor);
      }
    } else if (msg->bus == 2U) {
      if (addr == BYD_SONG_ACC_HUD_ADAS) {
        uint8_t accstate = (uint8_t)((msg->data[2] >> 3) & 0x07U);  // AccState : 19|3@1+
        bool cruise_engaged = (accstate == 3U) || (accstate == 5U); // 3 = ACC active, 5 = driver override
...
static bool byd_tx_hook(const CANPacket_t *msg) {
  if (byd_song_mode) {
    if (msg->bus == 0U) {
      if (msg->addr == BYD_SONG_ACC_MPC_STATE) {
        // LKAS_Output : 16|11@1+ in raw torque units (STEER_MAX = 300); mirrors
        // SongCarControllerParams. The EPS's own MainTorque frames the measurement.
        const TorqueSteeringLimits BYD_SONG_STEERING_LIMITS = {
          .max_torque = 300,
          .max_rate_up = 9,
          .max_rate_down = 9,
          .max_torque_error = 80,
        ...
        int desired_torque = (int)((msg->data[3] & 0x07U) << 8) | (int)msg->data[2];
        desired_torque = to_signed(desired_torque, 11);
        // LKAS_Active : 28|1@0+; the EPS's own CruiseActivated counts as a request too,
        // so a stale camera cruise state can't cut torque we're still commanding.
        bool steer_req = GET_BIT(msg, 28U) || byd_eps_cruiseactivated;

        if (steer_torque_cmd_checks(desired_torque, steer_req, BYD_SONG_STEERING_LIMITS)) {
          return false;
        }
      }
    }
...
static bool byd_fwd_hook(int bus_num, int addr) {
  if (byd_song_mode) {
    // Static split: the faked EPS frame is for the camera only, and the LKAS/ACC frames
    // the camera sends must never reach the bus openpilot is transmitting on.
    if ((bus_num == 0) && ((uint32_t)addr == BYD_SONG_ACC_EPS_STATE)) {
      return true;
    }
    if ((bus_num == 2) && (((uint32_t)addr == BYD_SONG_ACC_MPC_STATE) || ((uint32_t)addr == BYD_SONG_ACC_CMD))) {
      return true;
    }
    return false;
  }
