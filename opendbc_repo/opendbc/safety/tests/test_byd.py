...
from opendbc.car.byd.values import BydSafetyFlags, LKAS_HUD_PASSTHROUGH
from opendbc.car.structs import CarParams
from opendbc.car.structs import CarControlSP, CarParamsSP
...
LKAS_HUD_ADAS = 0x316
ACC_CMD = 0x32E  # blocked: not in the ATTO3 TX allowlist
ACC_HUD_ADAS = 0x32D

# SONG PLUS DMI (byd_song.dbc) message addresses
SONG_ACC_MPC_STATE = 0x316  # LKAS torque command, bus 0
SONG_ACC_EPS_STATE = 0x318  # EPS state, bus 0; faked back on bus 2
SONG_ACC_HUD_ADAS = 0x32D  # ACC state, camera on bus 2
SONG_ACC_CMD = 0x32E  # ACC command, bus 0
SONG_CARSPEED = 0x121  # dash speed, bus 0
SONG_PEDAL = 0x342  # GAS_PEDAL/BRAKE_PEDAL, bus 0
...
class TestBydSafety(TestBydSafetyBase):
  ...
  def test_template_is_constant_over_a_steering_episode(self):
    ...
    cc = CarController({Bus.pt: "byd_general"}, CP, CarParamsSP())
    ...
    for _ in range(2000 * CarControllerParams(CP).STEER_STEP):  # 2000 commands, ~40 s at 50 Hz
      _, can_sends = cc.update(CC, CarControlSP(), CS, 0)
  ...
  def test_acc_cmd_not_allowed(self):
    """Longitudinal is stock-only on the ATTO3; ACC_CMD must never be transmitted."""
    self.safety.set_controls_allowed(True)
    values = {"ACCEL_CMD": 100}
    self.assertFalse(self._tx(self.packer.make_can_msg_safety("ACC_CMD", MAIN_BUS, values)))


class TestBydSongSafety(common.CarSafetyTest, common.MotorTorqueSteeringSafetyTest):
  """SONG PLUS DMI: torque steering on ACC_MPC_STATE (bus 0), faked ACC_EPS_STATE (bus 2)."""
  TX_MSGS = [[SONG_ACC_MPC_STATE, MAIN_BUS],
             [SONG_ACC_EPS_STATE, CAM_BUS],
             [SONG_ACC_CMD, MAIN_BUS]]

  RELAY_MALFUNCTION_ADDRS = {MAIN_BUS: (SONG_ACC_MPC_STATE,)}
  FWD_BLACKLISTED_ADDRS = {CAM_BUS: [SONG_ACC_MPC_STATE, SONG_ACC_CMD],
                           MAIN_BUS: [SONG_ACC_EPS_STATE]}
  ...
  def setUp(self):
    self.packer = CANPackerSafety("byd_song")
    self.safety = libsafety_py.libsafety
    self.safety.set_safety_hooks(CarParams.SafetyModel.byd, BydSafetyFlags.SONG_PLUS_DMI)
    self.safety.init_tests()

  def _torque_meas_msg(self, torque):
    values = {"MainTorque": torque}
    return self.packer.make_can_msg_safety("ACC_EPS_STATE", MAIN_BUS, values)

  def _torque_cmd_msg(self, torque, steer_req=1):
    values = {"LKAS_Output": torque, "LKAS_Active": steer_req}
    return self.packer.make_can_msg_safety("ACC_MPC_STATE", MAIN_BUS, values)

  def _speed_msg(self, speed):
    values = {"CarDisplaySpeed": speed}
    return self.packer.make_can_msg_safety("CARSPEED", MAIN_BUS, values)
  ...
  def _user_brake_msg(self, brake):
    ...
    return self.packer.make_can_msg_safety("PEDAL", MAIN_BUS, values)
  ...
  def _pcm_status_msg(self, enable):
    ...
    return self.packer.make_can_msg_safety("ACC_HUD_ADAS", CAM_BUS, values)

  # The SONG rx path parses neither a MADS button message nor an ACC main-switch
  # message (AccOn1 stays unused, only AccState gates pcm_cruise_check), so the
  # MADS tests that need them are skipped like on the ATTO3.
